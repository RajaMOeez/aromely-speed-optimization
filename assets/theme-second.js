  ...
  theme.Variants = (function () {
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _onSelectChange: function ({ srcElement }) {
        ...
        // 1. CHANGE COMMENT ADDED
        let titleSelector = document.querySelectorAll(".opti_vt_label");
        if (titleSelector) {
          titleSelector.forEach(selector => {
            selector.innerHTML = this.currentVariant.title
          })
        }
        ...
        // CHANGE COMMENT ADDED 14-08-2024
        let stickyInputs = document.querySelectorAll(".sticky_form_input");
        if (stickyInputs) {
        ...
        // CHANGE COMMENT ADDED 12-08-2024
        let savingLabel = document.querySelectorAll("[data-price-saving]");
        let ps_labels = document.querySelectorAll(".opti_ps_label");
        if (savingLabel && ps_labels) {
          savingLabel.forEach(label => {
            label.innerHTML = ps_labels[0].innerHTML;
          })
        }
      },
  ...
  theme.CartForm = (function () {
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      cartMarkup: function (text) {
        ...
        // OPTIEXPERTS -> UPDATING PROGRESS BARS HERE
        // {% comment %} AROMELY -> CODE TO MOVE {% endcomment %}
        let progressNumbers = document.querySelector("#progress_numbers");
        let progressMoney = document.querySelector("#progress_money");
        let continueMessage = document.querySelector(".progress_cont_message");
        let endMessage = document.querySelector(".progress_end_message")

        if(progressNumbers) {
          progressNumbers.value = count;
          let maxRange = progressNumbers.dataset.maxRange;
          if(count > maxRange) {
            continueMessage.classList.remove("show_label");
            endMessage.classList.add("show_label");
          } else {
            continueMessage.classList.add("show_label");
            endMessage.classList.remove("show_label");
          }
        }
        if(progressMoney) {
          progressMoney.value = subtotal / 100;
          let maxRange = progressMoney.dataset.money;
          if(subtotal > maxRange) {
            continueMessage.classList.remove("show_label");
            endMessage.classList.add("show_label");
          } else {
            continueMessage.classList.add("show_label");
        ...
        theme.cart.changeItem(key, qty)
          .then(function (cart) {
            ...
            if (parsedCart.status === 422) {
              ...
              // REMOVE CHILD PRODUCTS RELATED TO THE PARENT BUNDLE
              let elPackageProp = el.getAttribute("bundle-pack-item");

                if (elPackageProp && qty <= 0 || qty === undefined) {
                    let updates = {};
                    parsedCart.items.forEach(item => {
                        if (item.properties && item.properties.bundle_pack === elPackageProp) {
                            updates[item.key] = 0;
                        }
                    });
              ...
              // DUPLICATE THE ITEMS IF QUANITY INCREASED
            //   if (elPackageProp && qty > 1) {
            //     const uniqueSuffix = new Date().getTime();
            //     let parentItem = parsedCart.items.find(
            //         item => item.properties?.bundle_pack === elPackageProp
            //     );
            
            //     if (parentItem) {
            //         let propProperty = elPackageProp + '-' + uniqueSuffix.toString().slice(-4);
            //         fetch('/cart/add.js', {
            //         method: 'POST',
            //         headers: {
            //             'Content-Type': 'application/json',
            ...
            //             quantity: 1,
            //             properties: {
            //             bundle_pack: propProperty
            //             }
            //         })
            //         });
            //         parsedCart.items.forEach(item => {
            //         if (
            //             item.properties?.bundle_pack === elPackageProp &&
            //             item.options_with_values[0]?.name === 'Samples'
            //         ) {
            //             fetch('/cart/add.js', {
            //             method: 'POST',
            //             headers: {
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      cacheElements: function () {
        this.cache = {
          ...
          price: document.querySelectorAll(this.selectors.price),
          comparePrice: document.querySelectorAll(this.selectors.comparePrice),
          // CHANGE COMMENT ADDED 12-08-2024
          ...
          // CHANGE COMMENT ADDED 06-08-2024
      ...
      updatePrice: function (evt) {
        ...
        if (variant) {
          ...
          this.cache.price.forEach(regPrice => {
            regPrice.innerHTML = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat);
          });

          // Sale price, if necessary
          // CHANGE COMMENT ADDED => 06-08-2024 => check this whole function for changings
          if (variant.compare_at_price > variant.price) {
            this.cache.comparePrice.forEach(compPrice => {
              compPrice.innerHTML = theme.Currency.formatMoney(variant.compare_at_price, theme.settings.moneyFormat);
              // this.cache.comparePrice.innerHTML = theme.Currency.formatMoney(variant.compare_at_price, theme.settings.moneyFormat);
            })
            this.cache.priceWrapper.classList.remove(classes.hidden);
            // this.cache.price.classList.add(classes.onSale);
            this.cache.price.forEach(regularPrice => {
              regularPrice.classList.add(classes.onSale);
            })
            ...
            this.cache.savePrice.forEach(savePriceDiscount => {
              savePriceDiscount.classList.remove(classes.hidden);
              savePriceDiscount.innerHTML = theme.strings.savePrice.replace('[saved_amount]', savings);
            })
          } else {
            ...
            this.cache.savePrice.forEach(savePriceDiscount => {
              savePriceDiscount.classList.add(classes.hidden);
            })
